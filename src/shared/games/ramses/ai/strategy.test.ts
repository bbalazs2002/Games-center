...
describe('chooseRamsesAiAction', () => {
  ...
  describe('EASY', () => {
    it('ignores the FULL memory (never avoids a known-wrong cell) — picks among slidable cells even when the full memory marks all of them as wrong', () => {
      let state = buildInteriorEmptyState({ activeCard: { kind: 'treasure', id: 'c1', treasureId: 'ankh', points: 2 } });
    ...
    it('takes an obvious win if the winning cell is within its short-term (recallEasy) window — see docs/ramses-0c-ai-specifikacio.md §3.3.2', () => {
      const state = buildInteriorEmptyState({ activeCard: { kind: 'treasure', id: 'c1', treasureId: 'ankh', points: 2 } });
...
describe('AI-only full game (smoke test)', () => {
  ...
  // 48-card real deck (empirically ~15,000-20,000 slides for a 30-card deck
  // in earlier testing, likely more now — that's an unbounded-tail random
  // process, not a hard guarantee — a fixed cap can never be 100% certain to
  // be enough without reseeding the RNG, which this module doesn't support).
  // The actual point of this test is that driving a real, varied slice of AI
  // decisions (blank reveals, wrong-treasure turn-passes, memory-informed
  // choices, occasional forgetting, AND now special-card naming/resolution)
