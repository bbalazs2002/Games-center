...
**Státusz:** Gwent-0a.1 (deck-építés) IMPLEMENTÁLVA (2026-07-31) — kártya-adatbázis/asset pipeline (5. szakasz), a `CardGrid` (újrahasználható ui-kit komponens) alapú frakció→vezér→deck-építő UI, `deckRules.ts` (pakli-validáció), localStorage-perzisztencia, `gamesRegistry` regisztráció, 26 vitest teszt + élő böngészős smoke teszt mind kész. Gwent-0a.2 (a tényleges parti-motor) MÉG NEM kezdődött el. Lásd 9. szakasz.
...
### 9.5 Felhasználói észrevételek — jegyzet (2026-07-31), MÉG NEM IMPLEMENTÁLVA

- **Frakció- és vezérváltás bármikor a deck-építés közben.** Jelenleg (`GwentSetupPage.tsx`) a folyamat lineáris: frakció → vezér → deck, és a "← Vezér"/"← Frakció" gombok visszalépnek, DE a frakcióváltás törli az addig összeállított `cardCounts`-ot (lásd `selectFaction`). A felhasználó ezt kényelmetlennek jelezte — jó lenne, ha a deck-építő lépésből bármikor válthatna frakciót/vezért anélkül, hogy (feleslegesen) elveszítené a más frakcióhoz tartozó, esetleg már kiválasztott lapjait, vagy legalábbis kevesebb kattintással lehessen visszalépni/váltani. Nincs még konkrét UI-terv hozzá — a következő Gwent-0a.1 utómunka-kör (vagy a 0a.2 UI-jának kialakításakor) döntendő el.
- **Hangok/zene a játékokhoz** — alacsony prioritású, játék-független kérés, lásd `Projekt-conception.md` legfelső "Hátralévő" listája. Gwent-re nézve ez leginkább a 2D kártya-renderer (0a.2, 6. szakasz) elkészültekor válik relevánssá (lapkijátszás/kör-lezárás hangeffektek).
