...
**Harmadik kör (2026-07-30, Ramses-0d, playtest alapján)**: `RAMSES_AI_MOVE_DELAY_MS` 500→1000ms (a felhasználó minden AI-lépés előtt érzékelhető "gondolkodási" szünetet kért); `findKnownPathToTarget` — BFS-alapú, memóriában megerősítetten üres mezőkön át egy megerősítetten ismert kincs-mezőig vezető útkeresés a HARD nehézségnél (valódi többlépéses terv az addigi egylépéses mohó választás helyett); `justVacatedCellId`/`deprioritize` — anti-oszcillációs logika (MEDIUM/HARD-nál, EASY-nél szándékosan nem, hogy a nehézségi létra meredeksége megmaradjon): a piramis, ami épp most jött egy mezőről, nem preferált cél a saját kör folytatásakor. Emellett a hot-seat AI szünete mostantól LÁTHATÓ is (nem csak néma várakozás) — lásd `ramses-0a-specifikacio.md` §9.2/§9.3 a teljes playtest-javítási körért, beleértve a feladás (FORFEIT) funkciót is, ami az AI döntéshozatalát is érinti (Sivatagi póker célpont-választásnál kizárja a feladott játékosokat, `strategy.ts`).
