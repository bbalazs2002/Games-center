...
**Utólagos kiegészítés (2026-07-30, Ramses-0d):** a fent kiemelt `replaceStringArray` helperben egy valós, kritikus hibát talált egy két valódi kliens közötti élő teszt — a `@colyseus/schema` saját `ArraySchema#splice()`-a nem enged üres tömbből nem-üresbe váltó splice-t, ami a `winnerIds` mező (mindig üresen indul, a játék végén válik nem-üressé) MINDEN szinkronizálását megszakította, tehát MINDEN online Ramses-parti befejezését (nem csak egy adott funkciót). Javítva, mindkét (Ramses ÉS Hotel) fogyasztót érintve — részletek: `ramses-0a-specifikacio.md` §9.4.
