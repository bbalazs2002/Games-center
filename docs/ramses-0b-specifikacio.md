...
**Kör-jelzés online módban:** mivel a tábla maga az egyetlen interakciós felület (nincs külön akció-menü, mint Hotelnél), fontos vizuálisan is jelezni, ha NEM a helyi kliens játékosa van soron (pl. a `handleCellClick` már ma is `slidableCellIds`-re szűr, ez változatlanul véd — csak UI-szinten kell jelezni, hogy "nem te jössz", nehogy a felhasználó azt higgye, elakadt a kattintás).
...
- [ ] **Kör-jelzés "nem te jössz vagy" UI — elég-e egy egyszerű szöveges státusz** (lásd 3.6), vagy legyen valami vizuálisabb (pl. a tábla elhalványítása, amíg nem a helyi játékos jön)? Ez tisztán UI-részlet, implementáció közben is eldönthető.
...
Első tervezési kör, 2026-07-27, a 4. szakasz mindkét érdemi nyitott kérdése eldőlt (a javasolt irány szerint) — a terv implementálásra kész volt, a kör-jelzés UI apró részletét ("X köre — várakozás…" egyszerű szöveg) implementáció közben döntöttük el.
