...
**Utólagos kiegészítés (real playtest bug, 2026-08-08, élesben rögzített naplóból igazolva — `hotel-659b938c-ba7e-4fac-9e3c-5860bb4c7bd3.jsonl` —, majd felhasználói review alapján általánosítva az Ingyen lépcső mezőre is):** az "Ingyen épület" mező (11. és 25. mező — a 11. pl. letoile-lal ÉS president-tel is szomszédos) landoláskor **NEM oldódott fel**, ha ugyanarra a mezőre épp egy MÁSIK hotelhez (itt: president) tartozó lépcső is épült, és azon éjszakai díj járt — az `applyRollMoveDice` (reducer.ts) a bérleti-díj-ellenőrzést (`staircaseLotWithPossibleRent`) VÉGEZTE EL ELŐSZÖR, és ha az igazat adott, azonnal visszatért `AWAITING_NIGHTS_ROLL` fázissal, mielőtt egyáltalán megnézte volna, hogy a mező típusa `FREE_BUILDING`. Ugyanez a hiba pontosan ugyanígy fennállt az "Ingyen lépcső" (`FREE_STAIRCASE`) mezőknél is. A mezők saját speciális akciója és a rajtuk esetleg álló, MÁSIK hotelhez tartozó lépcső bérleti díja egymástól független mechanika — **javítva, a helyes sorrend szerint:** dobás → +2000 mező → Ingyen épület/Ingyen lépcső → éjszakázás (bérleti díj) → kötelező árverezés (ha nem fizethető) → a `RESOLVING_SPACE` fázis szabad lépései (vásárlás/építkezés/lépcső vásárlása/árverezés indítása/kör vége). A Feladás (`FORFEIT`) ellenőrizve: ettől függetlenül, a játékos saját körének BÁRMELY fázisában elérhető, egyedül az árverés alatt nem (`canForfeit` — `state.turnPhase !== 'AUCTION_IN_PROGRESS'`), ez már eddig is helyesen működött.
