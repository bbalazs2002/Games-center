...
**Státusz:** Fázis 0a/0b/0c (Dáma) implementálva és élesben ellenőrizve; Fázis 2 Hotel-0a (helyi vertikum), Hotel-0b (multiplayer), Hotel-0c.1 (assetek/UI/animáció) és Hotel-0d (AI ellenfél, online ÉS hot-seat módban) implementálva és lezárva; Ramses-0a/0b/0c szintén implementálva és lezárva; Dáma-0d ÉS Dáma-0d.2 (AI nehézségi szintek + csak-AI hangolási kör, egy valódi determinisztikus-holtpont hibát is javítva) szintén implementálva és lezárva; a B-klaszter egységes vizuális nyelve ("Rács") szintén implementálva és élesben ellenőrizve; a játékfüggetlen UX-fejlesztések (4d — játékszabály-modál, hibabejelentő, Lobby/játékválasztó stílus, betöltőképernyő) szintén implementálva és élesben ellenőrizve; a Ramses speciális kártyái (8. szakasz, lásd [ramses-0a-specifikacio.md](./ramses-0a-specifikacio.md) §8) ÉS egy azt követő playtest-javítási kör (Ramses-0d — feladás funkció, AI-finomítás, UI polish, egy kritikus multiplayer szinkron-hiba javítása) szintén implementálva és élesben ellenőrizve (2026-07-30); **az éles béta telepítés (4e) megtörtént — mind a három játék (Dáma, Hotel, Ramses) élesben fut** a `balazs.gyserver.domenet.info/game-center` alatt. Hátralévő: Hotel-0c.2 (valódi 3D modellek, a felhasználó oldalán), Hotel-0d.2 (jövőbeli fázis: csak-AI játékok elemzése), valós dobozfotók a játékválasztóhoz (4d, a felhasználó oldalán).
...
4b. **Ramses — a Hotel-0c.2 (valódi 3D modellek, a felhasználó saját feladata) mellett, párhuzamosan indított oldal-ág, az E) klaszter ("rejtett információs") első tagja.** Nem a fő számozott sorrend része, hanem a te döntésed alapján közben megkezdett munka.
   ...
   - **Ramses speciális kártyái (Homokvihar, Ajándék, Kockázat, Sivatagi póker, Fata Morgana, Záró) — IMPLEMENTÁLVA és élesben ellenőrizve (2026-07-30).** A felhasználó pontosította a valós szabálykönyv szerinti kártyaszövegeket és biztosította a valódi, fényképezett assetek első verzióját; mind a 6 kártya, az `includeSpecialCards` ki/bekapcsolható kapcsoló (alapértelmezetten bekapcsolva), és a nevezési lépésekhez szükséges tárcsa-menü (`RamsesActionWheel.tsx`, a Hotel mintáját követve) elkészült. Részletek: [ramses-0a-specifikacio.md §8](./ramses-0a-specifikacio.md).
   - **Ramses-0d — playtest-javítási kör: IMPLEMENTÁLVA és élesben ellenőrizve (2026-07-30).** Több kör hibajavítás/finomítás valós böngészős (hot-seat, AI-ellenes ÉS két valódi bejelentkezett kliens közötti online) playteszt alapján: vizuális visszajelzés a speciális kártyákhoz/köráthelyezéshez (banner-rendszer), AI-finomítás (1s gondolkodási szünet, BFS-alapú célkeresés, anti-oszcilláció), és egy új **feladás (FORFEIT)** funkció (hot-seat ÉS multiplayer módban is — a soron lévő játékos kieshet, csak egy aktív játékos esetén ér véget a parti). **Egy valós, kritikus multiplayer szinkron-hibát is feltárt és javított**: a `@colyseus/schema` saját `ArraySchema#splice()`-a nem enged üres tömbből nem-üresbe váltó splice-t, ami MINDEN online Ramses-parti befejezésekor (nem csak feladásnál) elakasztotta a szinkronizálást — a megosztott `replaceStringArray` helper javítása a Hotel-t (`passedPlayerIds`/`lotsWithStaircasePurchasedThisTurn`) is érintette, ott ugyanez a hiba eddig lappangott. Emellett a natív `<select>`/checkbox HTML-elemek egyedi, témázott megfelelőkre cserélődtek mind a négy setup/lobby oldalon. Részletek: [ramses-0a-specifikacio.md §9](./ramses-0a-specifikacio.md), [shell-ux-specifikacio.md §11](./shell-ux-specifikacio.md).
...
4d. **Játékfüggetlen UX-fejlesztések — IMPLEMENTÁLVA és élesben (Playwright) ellenőrizve (2026-07-29), ugyanazon a napon, mint a tervezés.** Négy, egymástól független, de közös infrastruktúrán osztozó shell-szintű fejlesztés: (1) játékszabály-modál minden játékhoz a mód-választó oldalon, a spec-dokumentumok verbátim szövegéből átszerkesztve; (2) hibabejelentő/javaslat-küldő a menüből és játék közben is, játékon belülről a teljes elérhető kontextussal (állapot, játékosok, esemény napló) — ehhez Dáma és Ramses motorja is kapott egy minimális eseménynaplót (korábban csak Hotelnek volt), Ramses esetén a meglévő `@colyseus/schema` hálózati szinkronba is bekötve; (3) a Lobby (és a mód-választó oldal) felveszi az adott játék saját vizuális nyelvét egy új, `gameId`-alapú téma-mechanizmuson keresztül (a B-klaszter `clusterBTheme.module.css`-ének mintáját általánosítva), a főoldali játékválasztó viszont egy önálló, semleges, "üveg" hatású csempe-rács, egyelőre monogram-helyettesítő képekkel (valós dobozfotókra vár); (4) a betöltő képernyő a puszta szöveg helyett az adott játék stílusát veszi fel, pulzáló felirattal. `tsc`/`eslint`/`vitest` (218/218)/`vite build` mind tiszta, a hibabejelentés mindkét iránya (menü és játékon belül) valós adatbázis-sorral igazolva. Részletek: [shell-ux-specifikacio.md](./shell-ux-specifikacio.md) §10.
4e. **Éles béta telepítés — IMPLEMENTÁLVA és élesben fut (2026-07-29-től, Ramses 2026-07-30-tól).** Moduláris, `balazs.gyserver.domenet.info/game-center` alatt elérhető éles telepítés a felhasználó saját szerverén (Apache2 + Docker), egy `ENABLED_GAMES` kapcsolóval (nem játékonkénti külön konténer), egy központi, önálló `shared-postgres` konténerrel, GitHub Actions + SSH alapú, kézzel indított deploy-workflow-val. Kezdetben Dáma+Hotel, majd a Ramses-0d playtest-javítási kör lezárása után (2026-07-30) Ramses is bekerült az `ENABLED_GAMES` listába — mindhárom játék élesben fut. Részletek: [deployment-specifikacio.md](./deployment-specifikacio.md).
