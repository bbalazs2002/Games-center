...
4e. **Éles béta telepítés — TERVEZÉS (2026-07-29), implementáció még nem kezdődött el.** Dáma és Hotel béta-kész, Ramses még nem — a cél egy moduláris, `domain.hu/game-center` alatt elérhető éles telepítés a felhasználó saját szerverén (Apache2 + Docker), ami csak a kész játékokat teszi elérhetővé egy `ENABLED_GAMES` kapcsolóval (nem játékonkénti külön konténer), a meglévő Postgres-konténert használja (nem hoz létre újat), és GitHub Actions + SSH alapú, kézzel indított deploy-workflow-val kerül élesbe. Részletek: [deployment-specifikacio.md](./deployment-specifikacio.md).
